...
$createAjaxUrl = Url::to(['maestros-conceptos/create-ajax']);
$baseViewUrl = Url::to(['maestros-conceptos/view']);
$baseUpdateUrl = Url::to(['maestros-conceptos/update']);
$baseDeleteUrl = Url::to(['maestros-conceptos/delete']);
?>
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
                    <div class="input-group w-auto input-group-flat">
                        ...
                        <input type="text" class="form-control form-control-sm" id="maestro-concepto-search" placeholder="Buscar...">
                    </div>
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <a href="javascript:void(0);" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add_maestro_concepto"><i class="ti ti-plus me-1"></i>Agregar</a>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-nowrap bg-white border mb-0" id="maestros-conceptos-table">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Código</th>
...
<!-- Modal Agregar Maestro Concepto -->
<div class="modal fade" id="add_maestro_concepto">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="maestro-concepto-form-errors" class="alert alert-danger d-none"></div>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" id="btn-save-maestro-concepto">
...
$(document).ready(function() {
    ...
    $('#maestro-concepto-search').on('keyup', function() {
        table.search(this.value).draw();
    });

    $('#form-add-maestro-concepto').on('submit', function(e) {
        e.preventDefault();
        var \$form = $(this);
        var \$btn = $('#btn-save-maestro-concepto');
        var \$errors = $('#maestro-concepto-form-errors');
        ...
        if (!\$('#maestro-concepto-active').is(':checked')) {
            formData += '&MaestrosConceptos[active]=0';
        }
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    var modal = bootstrap.Modal.getInstance(document.getElementById('add_maestro_concepto'));
                    modal.hide();
                    \$form[0].reset();
                    $('#maestro-concepto-active').prop('checked', true);
                    var activeBadge = res.model.active ? '<span class="badge badge-soft-success">Sí</span>' : '<span class="badge badge-soft-danger">No</span>';
                    var categoryLabels = { ingreso: 'ingreso', deduccion: 'deduccion', aporte: 'aporte', provision: 'provision', otro: 'otro' };
                    var categoryLabel = categoryLabels[res.model.category] || res.model.category;
                    table.row.add([
                        res.model.id,
                        res.model.code || '-',
                        res.model.name,
    ...
    $('#add_maestro_concepto').on('hidden.bs.modal', function() {
        $('#form-add-maestro-concepto')[0].reset();
        $('#maestro-concepto-active').prop('checked', true);
        $('#maestro-concepto-form-errors').addClass('d-none').empty();
    });
});
JS;
