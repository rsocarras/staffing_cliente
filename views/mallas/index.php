        ...
        <div class="card mb-0">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-nowrap bg-white border mb-0" id="mallas-table">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Empresa</th>
...
<div class="modal fade" id="add_malla">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Agregar Malla</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i class="ti ti-circle-x"></i></button>
            </div>
            <?php
            $modelMallaModal = new \app\models\Mallas();
            $modelMallaModal->loadDefaultValues();
            $formMalla = \yii\widgets\ActiveForm::begin([
                'id' => 'form-add-malla',
                'action' => '',
                'method' => 'post',
            ...
            <div class="modal-body">
                <div id="malla-form-errors" class="alert alert-danger d-none"></div>
                <?= $this->render('_form_fields', [
                    'model' => $modelMallaModal,
                    'form' => $formMalla,
                ]) ?>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-light me-2" data-bs-dismiss="modal">Cancelar</button>
                <button type="submit" class="btn btn-primary" id="btn-save-malla">
...
$(document).ready(function() {
    ...
    $('#malla-search').on('keyup', function() {
        table.search(this.value).draw();
    });

    $('#form-add-malla').on('submit', function(e) {
        e.preventDefault();
        var \$form = $(this);
        var \$btn = $('#btn-save-malla');
        var \$errors = $('#malla-form-errors');
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    var modal = bootstrap.Modal.getInstance(document.getElementById('add_malla'));
                    modal.hide();
                    \$form[0].reset();
                    table.row.add([
                        res.model.id,
    ...
    $('#add_malla').on('hidden.bs.modal', function() {
        $('#form-add-malla')[0].reset();
        $('#malla-form-errors').addClass('d-none').empty();
    });
});
JS;
