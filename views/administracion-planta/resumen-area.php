...
<div class="page-wrapper">
    <div class="content">
        ...
        <?= $this->render('_filters', ['searchModel' => $searchModel, 'filterOptions' => $filterOptions, 'exportScope' => 'resumen-area']) ?>
        <?= $this->render('_kpis', ['kpis' => $dashboard['kpis']]) ?>

        <div class="card">
            <div class="card-body">
                <?= $this->render('_summary_table', ['rows' => $rows, 'tableId' => 'resumen-area-table']) ?>
            </div>
        </div>
    </div>
...
<?php
$this->registerJs("$('#resumen-area-table').DataTable({pageLength: 25, order: [[2,'asc']]});", \yii\web\View::POS_READY);
?>
