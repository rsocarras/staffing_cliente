...
$this->title = 'Contabilidad Centro Utilidad';
...
$createAjaxUrl = Url::to(['contabilidad-centro-utilidad/create-ajax']);
$baseViewUrl = Url::to(['contabilidad-centro-utilidad/view']);
$baseUpdateUrl = Url::to(['contabilidad-centro-utilidad/update']);
$baseDeleteUrl = Url::to(['contabilidad-centro-utilidad/delete']);
?>
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
                    <div class="input-group w-auto input-group-flat">
                        ...
                        <input type="text" class="form-control form-control-sm" id="centro-utilidad-search" placeholder="Buscar...">
                    </div>
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <a href="javascript:void(0);" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add_centro_utilidad"><i class="ti ti-plus me-1"></i>Agregar</a>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-nowrap bg-white border mb-0" id="contabilidad-centro-utilidad-table">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Código</th>
...
<!-- Modal Agregar Centro Utilidad -->
<div class="modal fade" id="add_centro_utilidad">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Agregar Centro de Utilidad</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i class="ti ti-circle-x"></i></button>
            </div>
            <?php
            $modelModal = new ContabilidadCentroUtilidad();
            $modelModal->loadDefaultValues();
            $formModal = \yii\widgets\ActiveForm::begin([
                'id' => 'form-add-centro-utilidad',
                'action' => '',
                'method' => 'post',
            ...
            <div class="modal-body">
                <div id="centro-utilidad-form-errors" class="alert alert-danger d-none"></div>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" id="btn-save-centro-utilidad">
...
$(document).ready(function() {
    var table = $('#contabilidad-centro-utilidad-table').DataTable({
        order: [[2, 'asc']],
        pageLength: 25,
    ...
    $('#centro-utilidad-search').on('keyup', function() {
        table.search(this.value).draw();
    });

    $('#form-add-centro-utilidad').on('submit', function(e) {
        e.preventDefault();
        var \$form = $(this);
        var \$btn = $('#btn-save-centro-utilidad');
        var \$errors = $('#centro-utilidad-form-errors');
        ...
        if (!\$('#centro-utilidad-activo').is(':checked')) {
            formData += '&ContabilidadCentroUtilidad[activo]=0';
        }
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    var modal = bootstrap.Modal.getInstance(document.getElementById('add_centro_utilidad'));
                    modal.hide();
                    \$form[0].reset();
                    $('#centro-utilidad-activo').prop('checked', true);
    ...
    $('#add_centro_utilidad').on('hidden.bs.modal', function() {
        $('#form-add-centro-utilidad')[0].reset();
        $('#centro-utilidad-activo').prop('checked', true);
        $('#centro-utilidad-form-errors').addClass('d-none').empty();
    });
});
JS;
