...
$baseViewUrl = Url::to(['cargos/view']);
$baseUpdateUrl = Url::to(['cargos/update']);
$baseDeleteUrl = Url::to(['cargos/delete']);
...
if ($profile && $profile->empresas_id) {
    $areasForSelect = ArrayHelper::map(
        Area::find()
            ...
            ->andWhere(['empresas_id' => $profile->empresas_id])
            ->orderBy('nombre')
            ->all(),
        'id',
        'nombre'
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
                    <div class="input-group w-auto input-group-flat">
                        ...
                        <input type="text" class="form-control form-control-sm" id="cargos-search" placeholder="Buscar...">
                    </div>
                    ...
                    <table class="table table-nowrap bg-white border mb-0" id="cargos-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?= $model->area ? Html::encode($model->area->nombre) : '-' ?></td>
                                <td><?= $model->subArea ? Html::encode($model->subArea->nombre) : '-' ?></td>
                                <td><?= Html::encode($model->codigo ?? '-') ?></td>
                                <td class="fw-medium text-dark"><?= Html::encode($model->nombre) ?></td>
                                <td><?= Html::encode($model->descripcion ?? '-') ?></td>
                                <td><?= $model->activo ? '<span class="badge bg-success">Sí</span>' : '<span class="badge bg-secondary">No</span>' ?></td>
                                <td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="cargo-form-errors" class="alert alert-danger d-none"></div>
...
$(document).ready(function() {
    ...
    $('#cargos-search').on('keyup', function() {
        table.search(this.value).draw();
    });

    function loadSubAreas(aid) {
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    ...
                    modal.hide();
                    \$form[0].reset();
                    $('#cargos-sub_area_id').html('<option value="">Seleccione sub-área</option>');
                    var activoBadge = res.model.activo ? '<span class="badge bg-success">Sí</span>' : '<span class="badge bg-secondary">No</span>';
                    table.row.add([
                        res.model.id,
                        res.model.area_nombre || '-',
                        res.model.sub_area_nombre || '-',
                        res.model.codigo || '-',
                        res.model.nombre,
