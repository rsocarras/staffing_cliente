...
(function () {
	...
	// Restaurar estado del sidebar desde localStorage (persistencia)
	(function restoreSidebarState() {
		const layout = document.documentElement.getAttribute('data-layout');
		if (layout === 'full-width' || layout === 'hidden') return;
		const saved = localStorage.getItem('__SIDEBAR_MINI__');
		if (saved === '1') {
	...
	$(document).on('click', '#toggle_btn, #toggle_btn2', function () {
		...
		if (isMini) {
			...
			localStorage.setItem('__SIDEBAR_MINI__', '0');
			setTimeout(function () {
			...
			localStorage.setItem('__SIDEBAR_MINI__', '1');
			setTimeout(function () {
				$(".header-left").removeClass("active");
			}, 100);
		}
